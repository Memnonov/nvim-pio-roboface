...
enum class EyePairState { IDLE, ACTIVE, DEBUG, CONV };
...
private:
  ...
  static const unsigned long avgBlinkDelay =
      1000; // Time between blinks, a *suggestion* more than a hard rule
...
public:
  ...
  void glance();
  void glance(float gDirX, float gDirY); // This is like a temporary "look this
                                         // much more in that direction" command
  void setPupilRadius(float newRad);
  void setMood(
      Moods Mood); // Note that valid values for moods are listed in the defines
  void showEyes();
